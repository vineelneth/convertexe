  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 bg-red-900/40 rounded-xl flex items-center justify-center"><PackageOpen size={20} className="text-rose-400" /></div>
        <div><h1 className="text-2xl font-bold text-white">Compress PDF</h1><p className="text-slate-400 text-sm">Reduce PDF file size by optimizing structure</p></div>
      </div>
      ...
      {result && status === 'completed' ? (
        <div className="card">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div><p className="font-semibold text-slate-200">PDF compressed!</p><p className="text-sm text-slate-400">{result.filename}</p></div>
          </div>
          <div className="bg-slate-800 rounded-lg p-4 mb-5 grid grid-cols-3 gap-4">
            ...
            <div><p className="text-xs text-slate-400 mb-1">Reduction</p><p className={`font-semibold ${reduction > 0 ? 'text-emerald-400' : 'text-slate-400'}`}>{reduction > 0 ? `-${reduction}%` : 'No change'}</p></div>
          </div>
